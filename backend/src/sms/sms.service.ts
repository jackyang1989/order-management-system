...
export class SmsService {
     ...
     * 发送验证码
     */
    async sendCode(dto: SendSmsCodeDto, ip?: string): Promise<{
        success: boolean;
        message: string;
        expireAt?: Date;
    }> {
     ...
     * 实际项目中对接阿里云、腾讯云等短信服务
     */
    private async doSendSms(phone: string, content: string, type: string, ip?: string): Promise<{
        success: boolean;
        msgId?: string;
        errorMsg?: string;
    }> {
     ...
     * 检查验证码是否有效（不消耗）
     */
    async checkCodeValid(phone: string, code: string, type: SmsCodeType): Promise<boolean> {
        const smsCode = await this.codeRepository.findOne({
            where: {
                phone,
                type,
