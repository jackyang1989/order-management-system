...
import {
  ...
  TaskKeyword,
  CreateTaskGoodsDto,
  CreateTaskKeywordDto,
} from './task-goods.entity';
...
export class TaskGoodsService {
  ...
  // ============ 任务商品 ============
  async createTaskGoods(
    taskId: string,
    dto: CreateTaskGoodsDto,
  ): Promise<TaskGoods> {
    ...
    const taskGoods = this.taskGoodsRepository.create({
      ...
      num: dto.num || 1,
      totalPrice,
      orderSpecs: dto.orderSpecs ? JSON.stringify(dto.orderSpecs) : undefined,
      verifyCode: dto.verifyCode,
    });
    return this.taskGoodsRepository.save(taskGoods);
  }

  async createTaskGoodsBatch(
    taskId: string,
    items: CreateTaskGoodsDto[],
  ): Promise<TaskGoods[]> {
  ...
  // ============ 任务关键词 ============
  async createTaskKeyword(
    taskId: string,
    dto: CreateTaskKeywordDto,
  ): Promise<TaskKeyword> {
  ...
  async createTaskKeywordBatch(
    taskId: string,
    keywords: CreateTaskKeywordDto[],
  ): Promise<TaskKeyword[]> {
    const taskKeywords = keywords.map((dto) =>
      this.taskKeywordRepository.create({
        taskId,
        ...dto,
      }),
    );
