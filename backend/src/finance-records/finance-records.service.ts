...
export class FinanceRecordsService {
    ...
    async findAll(filter: FinanceRecordFilterDto): Promise<{
        ...
        if (filter.userType !== undefined) {
            queryBuilder.andWhere('fr.userType = :userType', { userType: filter.userType });
        }
        if (filter.moneyType !== undefined) {
            queryBuilder.andWhere('fr.moneyType = :moneyType', { moneyType: filter.moneyType });
        }
        if (filter.financeType !== undefined) {
            queryBuilder.andWhere('fr.financeType = :financeType', { financeType: filter.financeType });
        }
     ...
     * 查询用户余额记录（买手押金流水）
     */
    async findUserBalanceRecords(userId: string, filter?: FinanceRecordFilterDto): Promise<{
        data: FinanceRecord[];
        total: number;
    }> {
        return this.findAll({
     ...
     * 查询用户银锭记录（买手礼金流水）
     */
    async findUserSilverRecords(userId: string, filter?: FinanceRecordFilterDto): Promise<{
        data: FinanceRecord[];
        total: number;
    }> {
        return this.findAll({
     ...
     * 查询商家押金记录
     */
    async findMerchantBalanceRecords(merchantId: string, filter?: FinanceRecordFilterDto): Promise<{
        data: FinanceRecord[];
        total: number;
    }> {
        return this.findAll({
     ...
     * 查询商家银锭记录
     */
    async findMerchantSilverRecords(merchantId: string, filter?: FinanceRecordFilterDto): Promise<{
        data: FinanceRecord[];
        total: number;
    }> {
        return this.findAll({
    ...
    async recordBuyerRecharge(
        ...
        return this.create({
            ...
            userType: FinanceUserType.BUYER,
            moneyType,
            financeType: moneyType === FinanceMoneyType.BALANCE
    ...
    async recordAdminOperation(
        ...
        if (userType === FinanceUserType.BUYER) {
            financeType = isDeduct ? FinanceType.BUYER_ADMIN_DEDUCT : FinanceType.BUYER_ADMIN_ADD;
        } else {
            financeType = isDeduct ? FinanceType.MERCHANT_ADMIN_DEDUCT : FinanceType.MERCHANT_ADMIN_ADD;
        }
