...
export class GoodsService {
  ...
  async findAll(
    ...
    if (filter?.search) {
      queryBuilder.andWhere('goods.name LIKE :search', {
    ...
    if (filter?.minPrice !== undefined && filter?.maxPrice !== undefined) {
      ...
      queryBuilder.andWhere('goods.showPrice >= :minPrice', {
        minPrice: filter.minPrice,
      });
    } else if (filter?.maxPrice !== undefined) {
      queryBuilder.andWhere('goods.showPrice <= :maxPrice', {
        maxPrice: filter.maxPrice,
      });
    }
 ...
 *
  ...
  async adminUpdate(
    ...
    try {
      ...
      // 5. 记录日志
      console.log(
        `[AdminLog] 修改商品 - 管理员${operatorName}操作: 修改商品id为:${goodsId}, 名称为:${goods.name}`,
      );
