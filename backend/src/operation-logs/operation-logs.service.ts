...
import { OperationLog, CreateOperationLogDto, QueryOperationLogDto, OperationType } from './operation-log.entity';
...
export class OperationLogsService {
    ...
    async create(dto: CreateOperationLogDto): Promise<OperationLog> {
        const log = this.operationLogRepository.create({
            ...
            requestData: dto.requestData ? JSON.stringify(dto.requestData) : undefined,
            responseData: dto.responseData ? JSON.stringify(dto.responseData) : undefined,
     ...
     * 查询操作日志列表
     */
    async findAll(query: QueryOperationLogDto): Promise<{ list: OperationLog[]; total: number }> {
        const { module, type, operatorName, startDate, endDate, page = 1, pageSize = 20 } = query;
        ...
        if (startDate && endDate) {
            where.createdAt = Between(new Date(startDate), new Date(endDate + ' 23:59:59'));
        } else if (startDate) {
