...
export class DingdanxiaService {
    ...
    private async httpRequest(
        ...
        const apiKey = await this.getApiKey();
        if (!apiKey) {
            throw new HttpException('订单侠 API Key 未配置', HttpStatus.SERVICE_UNAVAILABLE);
        }
    ...
    async validateGoodsLink(
        ...
        if (extractedId) {
            ...
            return {
                ...
                error: isValid ? undefined : `商品ID不匹配，期望: ${expectedTaobaoId}，实际: ${extractedId}`,
            };
        }
        ...
        return {
            ...
            error: isValid ? undefined : `商品ID不匹配，期望: ${expectedTaobaoId}，实际: ${parseResult.numIid}`,
        };
    }
