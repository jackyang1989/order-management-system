...
export class MerchantBankCardsController {
  ...
  async findAll(@Request() req) {
    const cards = await this.bankCardsService.findAllByMerchant(
      req.user.userId,
    );
  ...
  async create(@Body() createDto: CreateMerchantBankCardDto, @Request() req) {
    try {
      const card = await this.bankCardsService.create(
        req.user.userId,
        createDto,
      );
      return {
  ...
  async getAllCards(
    ...
    if (status !== undefined)
      filters.status = parseInt(status) as MerchantBankCardStatus;
