...
export class UserAddressesController {
  ...
  async findAll(@Request() req) {
    const addresses = await this.addressesService.findAllByUser(
      req.user.userId,
    );
  ...
  async getDefault(@Request() req) {
    const address = await this.addressesService.getDefaultAddress(
      req.user.userId,
    );
  ...
  async setDefault(@Param('id') id: string, @Request() req) {
    try {
      const address = await this.addressesService.setDefault(
        id,
        req.user.userId,
      );
      return {
