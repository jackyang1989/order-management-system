...
import { Notice, NoticeRead, NoticeStatus, NoticeTarget, CreateNoticeDto, UpdateNoticeDto } from './notice.entity';
...
export class NoticesService {
    ...
    async findPublished(
        ...
        const queryBuilder = this.noticeRepository.createQueryBuilder('notice')
            ...
            .andWhere('(notice.expiredAt IS NULL OR notice.expiredAt > :now)', { now });
     ...
     * 获取公告详情（用户端，增加浏览次数）
     */
    async findOneForUser(id: string, userId?: string, userType?: number): Promise<Notice | null> {
     ...
     * 获取弹窗公告
     */
    async findPopupNotices(target: NoticeTarget = NoticeTarget.ALL): Promise<Notice[]> {
        const now = new Date();
     ...
     * 获取未读公告数量
     */
    async getUnreadCount(userId: string, userType: number, target: NoticeTarget): Promise<number> {
        const now = new Date();
     ...
     * 标记为已读
     */
    async markAsRead(noticeId: string, userId: string, userType: number): Promise<void> {
     ...
     * 标记所有为已读
     */
    async markAllAsRead(userId: string, userType: number, target: NoticeTarget): Promise<void> {
        const now = new Date();
     ...
     * 创建公告
     */
    async create(createDto: CreateNoticeDto, adminId?: string, adminName?: string): Promise<Notice> {
