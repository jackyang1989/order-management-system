...
export class DayCountsService {
  ...
  async incrementUserDayCount(
    ...
    if (increment.completedCount)
      record.completedCount += increment.completedCount;
    if (increment.cancelledCount)
      record.cancelledCount += increment.cancelledCount;
    if (increment.totalAmount)
      record.totalAmount = Number(record.totalAmount) + increment.totalAmount;
    if (increment.commissionEarned)
      record.commissionEarned =
        Number(record.commissionEarned) + increment.commissionEarned;
    if (increment.commissionPaid)
      record.commissionPaid =
        Number(record.commissionPaid) + increment.commissionPaid;
  ...
  async getUserStats(
    ...
    const queryBuilder = this.userDayCountRepository
      .createQueryBuilder('c')
      .where('c.userId = :userId', { userId });

    if (filter?.startDate) {
      queryBuilder.andWhere('c.date >= :startDate', {
    ...
    const data = await queryBuilder.orderBy('c.date', 'DESC').getMany();
   ...
   * 增加平台当日统计
   */
  async incrementPlatformStat(
    field: keyof PlatformDayStat,
    amount: number = 1,
  ): Promise<void> {
  ...
  async getPlatformStats(filter?: DayCountFilterDto): Promise<{
    ...
    if (filter?.startDate) {
      queryBuilder.andWhere('s.date >= :startDate', {
        startDate: filter.startDate,
      });
    }
    if (filter?.endDate) {
    ...
    const data = await queryBuilder.orderBy('s.date', 'DESC').getMany();
