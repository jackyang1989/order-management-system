...
export class MerchantBlacklistService {
    ...
    // 获取商家的黑名单列表
    async findAll(sellerId: string, filter?: BlacklistFilterDto): Promise<{
        ...
        const queryBuilder = this.blacklistRepository.createQueryBuilder('bl')
    ...
    // 创建黑名单记录
    async create(sellerId: string, dto: CreateBlacklistDto): Promise<MerchantBlacklist> {
        // 检查是否已存在
    ...
    // 更新黑名单记录
    async update(id: string, sellerId: string, dto: UpdateBlacklistDto): Promise<MerchantBlacklist> {
