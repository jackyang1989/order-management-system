...
import { CreateBlacklistDto, UpdateBlacklistDto, BlacklistFilterDto } from './merchant-blacklist.entity';
...
export class MerchantBlacklistController {
    ...
    @Put(':id')
    async update(@Request() req, @Param('id') id: string, @Body() dto: UpdateBlacklistDto) {
        const sellerId = req.user.sub;
    ...
    async checkBlacklist(@Request() req, @Param('accountName') accountName: string) {
        const sellerId = req.user.sub;
        const isBlacklisted = await this.blacklistService.isBlacklisted(sellerId, accountName);
