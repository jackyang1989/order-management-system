...
export class SensitiveWordsService implements OnModuleInit {
  ...
  async checkText(
    ...
    if (hasSensitive && !blocked) {
      for (const wordObj of matchedWordObjects) {
        const replacement =
          wordObj.replacement || '*'.repeat(wordObj.word.length);
  ...
  async findAll(filter?: SensitiveWordFilterDto): Promise<{
    ...
    if (filter?.keyword) {
      queryBuilder.andWhere('w.word LIKE :keyword', {
  ...
  async updateWord(
    id: string,
    dto: Partial<CreateSensitiveWordDto>,
  ...
  async getStats(): Promise<{
    ...
    const activeWords = await this.wordRepository.count({
      where: { isActive: true },
    });
  ...
  async initDefaultWords(): Promise<void> {
    const defaultWords = [
      ...
      {
        word: '赌博',
        type: SensitiveWordType.GAMBLING,
        level: SensitiveLevel.HIGH,
      },
      {
        word: '博彩',
    ...
    for (const w of defaultWords) {
      const exists = await this.wordRepository.findOne({
        where: { word: w.word },
      });
      if (!exists) {
