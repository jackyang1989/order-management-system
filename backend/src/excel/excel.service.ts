...
export class ExcelService {
  ...
  async exportOrders(filter: {
    ...
    if (filter.merchantId) {
      queryBuilder.andWhere(
        'o.taskId IN (SELECT id FROM tasks WHERE merchantId = :merchantId)',
        {
  ...
  async exportTasks(filter: {
    ...
    if (filter.merchantId) {
      queryBuilder.andWhere('t.merchantId = :merchantId', {
        merchantId: filter.merchantId,
      });
    }
    if (filter.startDate && filter.endDate) {
   ...
   * 解析发货Excel
   */
  async parseDeliveryExcel(buffer: Buffer): Promise<
    Array<{
      orderId?: string;
  ...
  > {
    ...
    for (const [colNum, header] of Object.entries(headers)) {
      ...
      if (header.includes('淘宝订单号') || header.includes('taobao'))
        taobaoOrderNoCol = col;
      if (header.includes('快递公司') || header === 'delivery')
        deliveryCol = col;
      if (header.includes('快递单号') || header === 'deliveryNum')
        deliveryNumCol = col;
    }
    ...
    worksheet.eachRow((row, rowNumber) => {
      ...
      const orderId = orderIdCol
        ? String(row.getCell(orderIdCol).value || '').trim()
        : undefined;
      const taobaoOrderNo = taobaoOrderNoCol
        ? String(row.getCell(taobaoOrderNoCol).value || '').trim()
        : undefined;
      const delivery = String(row.getCell(deliveryCol).value || '').trim();
      const deliveryNum = String(
        row.getCell(deliveryNumCol).value || '',
      ).trim();
