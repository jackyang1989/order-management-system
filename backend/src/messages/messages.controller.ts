...
export class MessagesController {
  ...
  async findMyMessages(@Request() req, @Query() filter: MessageFilterDto) {
    ...
    const result = await this.messagesService.findUserMessages(
      req.user.userId,
      userType,
      filter,
    );
  ...
  async batchDelete(@Body() body: { ids: string[] }, @Request() req) {
    const count = await this.messagesService.batchDelete(
      body.ids,
      req.user.userId,
    );
  ...
  async getUnreadCountByType(@Request() req) {
    const userType =
      req.user.role === 'merchant'
        ? MessageUserType.MERCHANT
        : MessageUserType.BUYER;
    const counts = await this.messagesService.getUnreadCountByType(
      req.user.userId,
      userType,
    );
