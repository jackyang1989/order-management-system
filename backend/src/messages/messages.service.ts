...
export class MessagesService {
  ...
  async getUnreadCountByType(
    userId: string,
    userType: MessageUserType,
  ): Promise<Record<MessageType, number>> {
    ...
    for (const type of Object.values(MessageType).filter(
      (v) => typeof v === 'number',
    )) {
  ...
  async initDefaultTemplates(): Promise<void> {
    const templates = [
      ...
      },
      {
        ...
        title: '订单已完成',
        content:
          '订单 {orderNo} 已完成，佣金 {commission} 元已发放至您的账户。',
        type: MessageType.ORDER,
      },
      {
      ...
      },
      {
        ...
        title: '有新任务发布',
        content:
          '新任务已发布：{taskTitle}，商品价格 {price} 元，佣金 {commission} 元。',
