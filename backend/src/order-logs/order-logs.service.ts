...
export class OrderLogsService {
    ...
    async logRefundProcess(
        ...
        return this.create({
            orderId,
            orderNo,
            action: approved ? OrderLogAction.REFUND_APPROVE : OrderLogAction.REFUND_REJECT,
            operatorType: OrderLogOperatorType.ADMIN,
            operatorId: adminId,
            operatorName: adminName,
            content: approved ? `同意退款${remark ? '：' + remark : ''}` : `拒绝退款${remark ? '：' + remark : ''}`,
     ...
     * 获取订单日志列表
     */
    async findByOrder(orderId: string, filter?: OrderLogFilterDto): Promise<OrderLog[]> {
