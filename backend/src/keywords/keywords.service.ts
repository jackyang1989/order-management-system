...
export class KeywordsService {
    ...
    async adminCreateScheme(
        ...
        try {
            ...
            // 过滤空关键词
            const validDetails = dto.details.filter(d => d.keyword && d.keyword.trim());
            ...
            // 创建关键词详情
            const details = validDetails.map(d => this.keywordDetailRepository.create({
            ...
            console.log(`[AdminLog] 添加关键词方案 - 管理员${operatorName}操作: 方案名称${dto.name}`);
    ...
    async adminUpdateScheme(
        ...
        try {
            ...
            const scheme = await this.goodsKeyRepository.findOne({ where: { id: schemeId } });
            if (!scheme) {
            ...
            console.log(`[AdminLog] 修改关键词方案 - 管理员${operatorName}操作: 方案ID${schemeId}, 名称${dto.name}`);
