...
export class KeywordsController {
    ...
    @Put('schemes/:id')
    async updateScheme(@Request() req, @Param('id') id: string, @Body() dto: UpdateGoodsKeyDto) {
        const sellerId = req.user.sub;
    ...
    async addKeywordDetail(
        ...
        const sellerId = req.user.sub;
        const detail = await this.keywordsService.addKeywordDetail(schemeId, sellerId, dto);
    ...
    async updateKeywordDetail(
        ...
        const sellerId = req.user.sub;
        const detail = await this.keywordsService.updateKeywordDetail(detailId, sellerId, dto);
        return { success: true, message: '关键词更新成功', data: detail };
    }

    @Delete('details/:detailId')
    async deleteKeywordDetail(@Request() req, @Param('detailId') detailId: string) {
        const sellerId = req.user.sub;
