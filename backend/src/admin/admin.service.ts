...
export class AdminService {
    ...
    async getUsers(page = 1, limit = 20, search?: string): Promise<{
        ...
        const data = await query
            .select(['user.id', 'user.username', 'user.phone', 'user.balance', 'user.status', 'user.createdAt'])
    ...
    // ============ 商家管理 ============
    async getMerchants(page = 1, limit = 20, status?: MerchantStatus): Promise<{
        data: Merchant[];
        total: number;
        page: number;
        totalPages: number;
        ...
        const total = await query.getCount();
        const data = await query
            .select(['merchant.id', 'merchant.username', 'merchant.phone', 'merchant.companyName', 'merchant.balance', 'merchant.status', 'merchant.createdAt'])
    ...
    async approveMerchant(id: string, approved: boolean): Promise<Merchant | null> {
        const merchant = await this.merchantsRepository.findOne({ where: { id } });
        if (!merchant) return null;
        merchant.status = approved ? MerchantStatus.APPROVED : MerchantStatus.REJECTED;
        return this.merchantsRepository.save(merchant);
    }

    // ============ 任务管理 ============
    async getTasks(page = 1, limit = 20, status?: number): Promise<{
        data: Task[];
        total: number;
        page: number;
        totalPages: number;
    ...
    // ============ 提现审核 ============
    async getWithdrawals(page = 1, limit = 20, status?: WithdrawalStatus): Promise<{
        data: Withdrawal[];
        total: number;
        page: number;
        totalPages: number;
    ...
    async approveWithdrawal(id: string, approved: boolean, remark?: string): Promise<Withdrawal | null> {
