...
export class UserInvitesController {
  ...
  async getMyInviteCode(@Request() req) {
    const userType =
      req.user.role === 'merchant' ? InviteType.MERCHANT : InviteType.BUYER;
    let code = await this.invitesService.getUserInviteCode(
      req.user.userId,
      userType,
    );
    ...
    if (!code) {
      // 自动生成邀请码
      code = await this.invitesService.generateInviteCode(
        req.user.userId,
        userType,
      );
    }
  ...
  async getMyInvites(@Request() req, @Query() filter: InviteFilterDto) {
    const result = await this.invitesService.findUserInvites(
      req.user.userId,
      filter,
    );
  ...
  async getMyInviter(@Request() req) {
    const invite = await this.invitesService.getInviteByInvitee(
      req.user.userId,
    );
    if (!invite) {
  ...
  async getRewardConfig(@Query('type') type: string) {
    const inviteType =
      type === 'merchant' ? InviteType.MERCHANT : InviteType.BUYER;
  ...
  async updateConfig(
    ...
    const inviteType =
      type === 'merchant' ? InviteType.MERCHANT : InviteType.BUYER;
    const config = await this.invitesService.updateRewardConfig(
      inviteType,
      dto,
    );
