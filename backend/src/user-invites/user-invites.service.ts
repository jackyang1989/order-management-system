...
export class UserInvitesService {
  ...
  async processInviteReward(inviteId: string): Promise<boolean> {
    ...
    if (invite.rewardAmount > 0) {
      const userType =
        invite.inviterType === InviteType.BUYER
  ...
  async findUserInvites(
    ...
    const stats = await this.inviteRepository
      ...
      .addSelect(
        'SUM(CASE WHEN i.status >= 1 THEN 1 ELSE 0 END)',
        'activatedCount',
      )
      .addSelect(
      ...
      .addSelect(
        'COALESCE(SUM(CASE WHEN i.status = 2 THEN i.rewardAmount ELSE 0 END), 0)',
        'totalReward',
      )
      .where('i.inviterId = :userId', { userId })
      .getRawOne();
  ...
  ): Promise<InviteRewardConfig | null> {
  ...
  async getInviteStats(): Promise<{
    ...
    const result = await this.inviteRepository
      ...
      .select('COUNT(*)', 'totalInvites')
      .addSelect(
        'SUM(CASE WHEN i.status = 0 THEN 1 ELSE 0 END)',
        'pendingCount',
      )
      .addSelect(
        'SUM(CASE WHEN i.status = 1 THEN 1 ELSE 0 END)',
