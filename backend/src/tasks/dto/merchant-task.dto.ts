...
export class MerchantTaskDto {
    ...
    // --- Browse Requirements (Task Steps) ---
    @IsBoolean()
    @IsOptional()
    ...
    @IsString()
    @IsOptional()
    gender?: string; // 性别要求 (New)

    @IsNumber()
    @IsOptional()
    ageMin?: number; // 年龄下限 (New)

    @IsNumber()
    @IsOptional()
    ageMax?: number; // 年龄上限 (New)

    @IsNumber()
    @IsOptional()
    repurchaseLimit?: number; // 重复购买限制 (1: 可复购, 0: 不可)

    @IsNumber()
    @IsOptional()
    buyLimit?: number; // 购买限制 (天数)
}
