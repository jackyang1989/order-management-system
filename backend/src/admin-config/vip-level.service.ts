...
export class VipLevelService implements OnModuleInit {
  ...
  async getUserVipPrivileges(
    ...
    return {
      dailyTaskLimit:
        type === 'buyer' ? vip.dailyTaskLimit : vip.publishTaskLimit,
   ...
   * 计算VIP佣金
   */
  calculateVipCommission(
    baseCommission: number,
    vipLevel: number,
    type: 'buyer' | 'merchant' = 'buyer',
  ): number {
   ...
   * 计算VIP服务费折扣
   */
  calculateVipServiceFee(
    baseFee: number,
    vipLevel: number,
    type: 'buyer' | 'merchant' = 'merchant',
  ): number {
    const cacheKey = `${type}-${vipLevel}`;
    const vip = this.vipCache.get(cacheKey);
