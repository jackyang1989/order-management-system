...
export class AdminUsersService {
    ...
    async findAllAdmins(page: number = 1, limit: number = 20): Promise<{
        data: AdminUser[];
        total: number;
        page: number;
        limit: number;
    ...
    async updateAdmin(id: string, updateDto: UpdateAdminUserDto): Promise<AdminUser> {
        const admin = await this.adminUserRepository.findOne({ where: { id } });
        if (!admin) {
    ...
    async updateRole(id: string, updateDto: Partial<AdminRole>): Promise<AdminRole> {
    ...
    async deleteRole(id: string): Promise<void> {
        ...
        // 检查是否有管理员使用此角色
        const adminCount = await this.adminUserRepository.count({ where: { roleId: id } });
        if (adminCount > 0) {
            throw new BadRequestException(`该角色下有 ${adminCount} 个管理员，无法删除`);
        }
    ...
    async findOperationLogs(
        page: number = 1,
        limit: number = 50,
        filters?: { adminId?: string; module?: string; startDate?: Date; endDate?: Date }
    ): Promise<{
    ...
    async checkPermission(adminId: string, permissionCode: string): Promise<boolean> {
        const admin = await this.adminUserRepository.findOne({ where: { id: adminId } });
        if (!admin || !admin.roleId) {
            return false;
        }

        const role = await this.adminRoleRepository.findOne({ where: { id: admin.roleId } });
        if (!role) {
            return false;
        }
