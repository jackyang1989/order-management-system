...
import {
    ...
    ConfirmReviewDto, CancelReviewDto, ReviewTaskFilterDto, ReviewTaskStatus,
    AdminReviewExamineDto, AdminBatchRefundDto
...
export class ReviewTasksController {
    ...
    async checkReviewable(@Param('orderId') orderId: string, @Request() req) {
        const result = await this.reviewTasksService.checkReviewable(orderId, req.user.userId);
        return { success: true, data: result };
    }
    ...
    async createReviewTask(@Body() dto: CreateReviewTaskDto, @Request() req) {
        try {
            const task = await this.reviewTasksService.createReviewTask(req.user.userId, dto);
    ...
    async payReviewTask(@Body() dto: PayReviewTaskDto, @Request() req) {
        try {
            const task = await this.reviewTasksService.payReviewTask(req.user.userId, dto);
    ...
    async cancelReviewTask(@Body() dto: CancelReviewDto, @Request() req) {
        try {
            const task = await this.reviewTasksService.cancelReviewTask(req.user.userId, dto);
    ...
    async confirmReview(@Body() dto: ConfirmReviewDto, @Request() req) {
        try {
            const task = await this.reviewTasksService.confirmReview(req.user.userId, dto);
            return { success: true, message: '确认完成，佣金已发放给买手', data: task };
        } catch (error) {
            return { success: false, message: error.message };
        }
    }
    ...
    async getMerchantTasks(
        ...
        const result = await this.reviewTasksService.findByMerchant(req.user.userId, filter);
        return { success: true, data: result };
    }
    ...
    async getUserTasks(
        ...
        const result = await this.reviewTasksService.findByUser(req.user.userId, filter);
        return { success: true, data: result };
    }
    ...
    async submitReview(@Body() dto: SubmitReviewDto, @Request() req) {
        try {
            const task = await this.reviewTasksService.submitReview(req.user.userId, dto);
            return { success: true, message: '追评提交成功，等待商家确认', data: task };
        } catch (error) {
            return { success: false, message: error.message };
        }
    }
    ...
    async rejectReview(@Body() dto: RejectReviewDto, @Request() req) {
        try {
            const task = await this.reviewTasksService.rejectByBuyer(req.user.userId, dto);
    ...
    async adminExamine(@Body() dto: AdminReviewExamineDto) {
        try {
            ...
            const message = dto.state === ReviewTaskStatus.APPROVED
                ? '审核通过，已通知买手'
                : '已拒绝';
    ...
    async adminBatchExamine(@Body() body: { reviewTaskIds: string[] }) {
        try {
            const result = await this.reviewTasksService.adminBatchExamine(body.reviewTaskIds);
            return {
                success: true,
    ...
    async adminBatchRefund(@Body() dto: AdminBatchRefundDto) {
        try {
            const result = await this.reviewTasksService.adminBatchRefund(dto.reviewTaskIds);
            return {
                success: true,
