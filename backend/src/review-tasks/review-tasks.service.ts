...
import {
    ReviewTask, ReviewTaskPraise, ReviewTaskStatus, ReviewPraiseType,
    CreateReviewTaskDto, PayReviewTaskDto, SubmitReviewDto, RejectReviewDto,
    ConfirmReviewDto, CancelReviewDto, ReviewTaskFilterDto, GoodsPraiseSettingDto,
...
import { FinanceUserType, FinanceMoneyType, FinanceType } from '../finance-records/finance-record.entity';
...
export class ReviewTasksService {
     ...
     */
    async getReviewableOrders(merchantId: string, page = 1, limit = 15): Promise<{
        list: Order[];
        total: number;
        page: number;
        limit: number;
     ...
     */
    async checkReviewable(orderId: string, merchantId: string): Promise<{ canReview: boolean; reason?: string; order?: Order }> {
        // 获取订单信息
     ...
     */
    async createReviewTask(merchantId: string, dto: CreateReviewTaskDto): Promise<ReviewTask> {
        // 验证商家VIP
        const merchant = await this.merchantsService.findOne(merchantId);
        if (!merchant) {
            throw new NotFoundException('商家不存在');
        }
        if (!merchant.vip || (merchant.vipExpireAt && new Date(merchant.vipExpireAt) < new Date())) {
            throw new BadRequestException('非VIP会员不能发布追评任务');
        }

        // 验证订单可追评
        const { canReview, reason, order } = await this.checkReviewable(dto.userTaskId, merchantId);
        if (!canReview) {
     ...
     */
    async payReviewTask(merchantId: string, dto: PayReviewTaskDto): Promise<ReviewTask> {
     ...
     * 只有 state < 3 时可以取消
     */
    async cancelReviewTask(merchantId: string, dto: CancelReviewDto): Promise<ReviewTask> {
        const reviewTask = await this.reviewTaskRepo.findOne({
            where: { id: dto.reviewTaskId, merchantId }
        });
        ...
        return await this.dataSource.transaction(async manager => {
            ...
            if (Number(reviewTask.yjprice) > 0) {
                const newBalance = Number(merchant.balance) + Number(reviewTask.yjprice);
                await manager.update('merchants', merchantId, { balance: newBalance });
            ...
            if (Number(reviewTask.ydprice) > 0) {
                const newSilver = Number(merchant.silver || 0) + Number(reviewTask.ydprice);
                await manager.update('merchants', merchantId, { silver: newSilver });
     ...
     */
    async submitReview(userId: string, dto: SubmitReviewDto): Promise<ReviewTask> {
        const reviewTask = await this.reviewTaskRepo.findOne({
     ...
     */
    async rejectByBuyer(userId: string, dto: RejectReviewDto): Promise<ReviewTask> {
        const reviewTask = await this.reviewTaskRepo.findOne({
            where: { id: dto.reviewTaskId, userId }
        });
        ...
        return await this.dataSource.transaction(async manager => {
            ...
            if (Number(reviewTask.yjprice) > 0) {
                const newBalance = Number(merchant.balance) + Number(reviewTask.yjprice);
                await manager.update('merchants', reviewTask.merchantId, { balance: newBalance });
            ...
            if (Number(reviewTask.ydprice) > 0) {
                const newSilver = Number(merchant.silver || 0) + Number(reviewTask.ydprice);
                await manager.update('merchants', reviewTask.merchantId, { silver: newSilver });
     ...
     * 发放佣金给买手，标记原订单已追评
     */
    async confirmReview(merchantId: string, dto: ConfirmReviewDto): Promise<ReviewTask> {
        const reviewTask = await this.reviewTaskRepo.findOne({
            where: { id: dto.reviewTaskId, merchantId }
        });
     ...
     */
    async adminExamine(reviewTaskId: string, state: number, remarks?: string): Promise<ReviewTask> {
        const reviewTask = await this.reviewTaskRepo.findOne({
     ...
     */
    async adminBatchExamine(reviewTaskIds: string[]): Promise<{ success: number; failed: number }> {
        let success = 0;
        let failed = 0;
     ...
     */
    async adminBatchRefund(reviewTaskIds: string[]): Promise<{ success: number; failed: number }> {
        let success = 0;
        let failed = 0;
     ...
     * 获取商家的追评任务列表
     */
    async findByMerchant(merchantId: string, filter: ReviewTaskFilterDto): Promise<{
        list: ReviewTask[];
        total: number;
        page: number;
        limit: number;
        ...
        if (filter.taskNumber) {
            queryBuilder.andWhere('rt.taskNumber LIKE :taskNumber', { taskNumber: `%${filter.taskNumber}%` });
        }
     ...
     * 获取买手的追评任务列表
     */
    async findByUser(userId: string, filter: ReviewTaskFilterDto): Promise<{
        list: ReviewTask[];
        total: number;
        page: number;
        limit: number;
     ...
     * 获取追评任务详情（含好评内容）
     */
    async findOneWithPraises(id: string): Promise<{ task: ReviewTask; praises: ReviewTaskPraise[] } | null> {
    ...
    async findAll(filter: ReviewTaskFilterDto): Promise<{
        ...
        if (filter.startDate) {
            queryBuilder.andWhere('rt.createdAt >= :startDate', { startDate: new Date(filter.startDate) });
        }
        if (filter.endDate) {
            queryBuilder.andWhere('rt.createdAt <= :endDate', { endDate: new Date(filter.endDate) });
        }
    ...
    async getStats(merchantId?: string): Promise<{
        ...
        const [unpaid, paid, approved, uploaded, completed, cancelled, rejected] = await Promise.all([
            this.reviewTaskRepo.count({ where: { ...baseWhere, state: ReviewTaskStatus.UNPAID } }),
            this.reviewTaskRepo.count({ where: { ...baseWhere, state: ReviewTaskStatus.PAID } }),
            this.reviewTaskRepo.count({ where: { ...baseWhere, state: ReviewTaskStatus.APPROVED } }),
            this.reviewTaskRepo.count({ where: { ...baseWhere, state: ReviewTaskStatus.UPLOADED } }),
            this.reviewTaskRepo.count({ where: { ...baseWhere, state: ReviewTaskStatus.COMPLETED } }),
            this.reviewTaskRepo.count({ where: { ...baseWhere, state: ReviewTaskStatus.CANCELLED } }),
            this.reviewTaskRepo.count({ where: { ...baseWhere, state: ReviewTaskStatus.REJECTED } }),
        ]);
