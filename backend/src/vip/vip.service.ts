...
export class VipService {
  ...
  async updatePackage(
    id: string,
    dto: Partial<CreateVipPackageDto>,
  ): Promise<VipPackage | null> {
  ...
  async purchaseVip(
    userId: string,
    dto: PurchaseVipDto,
  ): Promise<VipPurchase | { payUrl: string; orderNo: string }> {
    const pkg = await this.vipPackageRepository.findOne({
  ...
  private async createAlipayOrder(
    ...
    if (
      pendingOrder &&
      now - Number(pendingOrder.createTime) < this.ORDER_COOLDOWN_SECONDS
    ) {
      const remainingSeconds =
        this.ORDER_COOLDOWN_SECONDS - (now - Number(pendingOrder.createTime));
      const remainingMinutes = Math.ceil(remainingSeconds / 60);
      throw new BadRequestException(
        `上一单未支付，请等待${remainingMinutes}分钟后再次充值`,
      );
    }
  ...
  async getUserVipStatus(userId: string): Promise<{
    ...
    const daysRemaining =
      isVip && user.vipExpireAt
        ? Math.ceil(
            (user.vipExpireAt.getTime() - now.getTime()) /
              (24 * 60 * 60 * 1000),
          )
        : 0;
