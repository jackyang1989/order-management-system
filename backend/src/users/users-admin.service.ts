...
export class UsersAdminService {
    ...
    async findAll(query: UserQueryDto): Promise<{
        ...
        if (query.verifyStatus !== undefined) {
            qb.andWhere('u.verifyStatus = :verifyStatus', { verifyStatus: query.verifyStatus });
        }
     ...
     * 调整用户余额
     */
    async adjustBalance(id: string, dto: AdjustBalanceDto): Promise<{
        user: User;
        log: BalanceLog;
    }> {
        const user = await this.userRepo.findOne({ where: { id } });
    ...
    async setVip(id: string, days: number, level?: number): Promise<User> {
        ...
        user.vip = true;
        const now = new Date();
        const expireAt = user.vipExpireAt && user.vipExpireAt > now
    ...
    async exportUsers(query: UserQueryDto): Promise<{
        ...
        const exportData = result.data.map(u => ({
            ...
            本金余额: u.balance,
            银锭余额: u.silver,
            实名状态: ['未认证', '待审核', '已认证', '已拒绝'][u.verifyStatus] || '未知',
