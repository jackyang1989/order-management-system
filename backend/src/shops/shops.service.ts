...
export class ShopsService {
    ...
    async findAll(query: { status?: ShopStatus; sellerId?: string, page?: number, limit?: number }): Promise<{ list: Shop[]; total: number }> {
        const { status, sellerId, page = 1, limit = 10 } = query;
        const qb = this.shopsRepository.createQueryBuilder('shop')
    ...
    async create(sellerId: string, data: Partial<Shop>): Promise<Shop> {
        ...
        if (data.accountName) {
            ...
            if (exists && exists.sellerId !== sellerId) { // Allow same user re-bind? Or strict unique?
    ...
    async update(id: string, sellerId: string, data: Partial<Shop>): Promise<Shop> {
        const shop = await this.shopsRepository.findOne({ where: { id, sellerId } });
        if (!shop) throw new BadRequestException('店铺不存在');

        // If changing accountName, check unique
        if (data.accountName && data.accountName !== shop.accountName) {
            const exists = await this.shopsRepository.findOne({ where: { accountName: data.accountName } });
