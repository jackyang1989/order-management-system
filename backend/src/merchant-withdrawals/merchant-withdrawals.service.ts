...
export class MerchantWithdrawalsService {
    ...
    async create(merchantId: string, createDto: CreateMerchantWithdrawalDto): Promise<MerchantWithdrawal> {
        return this.withdrawalRepository.manager.transaction(async transactionalEntityManager => {
            // 1. 获取银行卡信息
            const bankCard = await this.bankCardsService.findOne(createDto.bankCardId, merchantId);
            if (!bankCard) {
                throw new NotFoundException('银行卡不存在');
            }

            if (createDto.amount < 100) {
                throw new BadRequestException('商家最低提现金额为100元');
            }
            ...
            // 3. 创建提现记录
            const withdrawal = transactionalEntityManager.create(MerchantWithdrawal, {
                merchantId,
    ...
    async getStats(merchantId: string): Promise<{
        ...
        const pending = await this.withdrawalRepository
            ...
            .andWhere('w.status = :status', { status: MerchantWithdrawalStatus.PENDING })
            .getRawOne();
        ...
        const completed = await this.withdrawalRepository
            ...
            .where('w.merchantId = :merchantId', { merchantId })
            .andWhere('w.status = :status', { status: MerchantWithdrawalStatus.COMPLETED })
            .getRawOne();
    ...
    async review(id: string, reviewDto: ReviewMerchantWithdrawalDto, adminId: string): Promise<MerchantWithdrawal> {
        return this.withdrawalRepository.manager.transaction(async transactionalEntityManager => {
            ...
            if (reviewDto.status === MerchantWithdrawalStatus.APPROVED ||
    ...
    async getAdminStats(): Promise<{
        ...
        const todayStats = await this.withdrawalRepository
            ...
            .select('COUNT(*)', 'count')
            .addSelect('SUM(w.amount)', 'amount')
            .where('w.status = :status', { status: MerchantWithdrawalStatus.COMPLETED })
