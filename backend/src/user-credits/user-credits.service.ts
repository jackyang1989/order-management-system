...
export class UserCreditsService {
  ...
  async changeCredit(
    ...
    if (credit.totalOrders > 0) {
      credit.completionRate =
        (credit.completedOrders / credit.totalOrders) * 100;
    }
  ...
  async getUserLogs(
    ...
    if (filter?.changeType) {
      queryBuilder.andWhere('l.changeType = :changeType', {
