...
export class UserCreditsController {
  ...
  async getMyCredit(@Request() req) {
    const userType =
      req.user.role === 'merchant'
        ? CreditUserType.MERCHANT
        : CreditUserType.BUYER;
  ...
  async getMyLogs(@Request() req, @Query() filter: CreditLogFilterDto) {
    const result = await this.creditsService.getUserLogs(
      req.user.userId,
      filter,
    );
  ...
  async getUserLogs(
    ...
    @Query() filter: CreditLogFilterDto,
  ) {
  ...
  async adjustCredit(
    ...
    const credit = await this.creditsService.adminAdjustCredit(
      userId,
      dto,
      req.user.userId,
    );
  ...
  async addToBlacklist(
    ...
    const credit = await this.creditsService.addToBlacklist(
      userId,
      dto,
      req.user.userId,
    );
  ...
  async removeFromBlacklist(@Param('userId') userId: string, @Request() req) {
    const credit = await this.creditsService.removeFromBlacklist(
      userId,
      req.user.userId,
    );
