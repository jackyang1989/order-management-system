...
export class RechargeService {
     ...
     * 生成充值订单号
     */
    private generateOrderNumber(userType: RechargeUserType, userId: string): string {
     ...
     * 创建充值订单（买手/商家端）
     */
    async createOrder(userId: string, userType: RechargeUserType, dto: CreateRechargeDto): Promise<Recharge> {
        if (dto.amount <= 0) {
     ...
     * 支付回调处理
     */
    async handleCallback(orderNumber: string, tradeNo: string, success: boolean): Promise<Recharge> {
     ...
     * 管理员直接充值
     */
    async adminRecharge(dto: AdminRechargeDto, operatorId: string): Promise<Recharge> {
        const queryRunner = this.dataSource.createQueryRunner();
        await queryRunner.connect();
        await queryRunner.startTransaction();
    ...
    private async updateUserBalance(
        ...
        if (userType === RechargeUserType.BUYER) {
            ...
            const merchant = await queryRunner.manager.findOne(Merchant, { where: { id: userId } });
            ...
            await this.financeRecordsService.recordAdminOperation(
                ...
                rechargeType === RechargeType.BALANCE ? FinanceMoneyType.BALANCE : FinanceMoneyType.SILVER,
                amount,
                balanceAfter,
                memo,
                operatorId || 'system'
    ...
    async findAll(filter: RechargeFilterDto): Promise<{
        ...
        if (filter.userType !== undefined) {
            queryBuilder.andWhere('r.userType = :userType', { userType: filter.userType });
        }
        if (filter.rechargeType !== undefined) {
            queryBuilder.andWhere('r.rechargeType = :rechargeType', { rechargeType: filter.rechargeType });
        }
    ...
    async findByUser(userId: string, userType: RechargeUserType): Promise<Recharge[]> {
