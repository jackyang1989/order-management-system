...
export class RechargeController {
  ...
  async createUserRecharge(@Request() req, @Body() dto: CreateRechargeDto) {
    ...
    const recharge = await this.rechargeService.createOrder(
      userId,
      RechargeUserType.BUYER,
      dto,
    );
    return {
      success: true,
  ...
  async getUserRechargeRecords(@Request() req) {
    const userId = req.user.sub;
    const records = await this.rechargeService.findByUser(
      userId,
      RechargeUserType.BUYER,
    );
  ...
  async alipayCallback(
    @Body() body: { orderNumber: string; tradeNo: string; success: boolean },
  ) {
    // 实际使用时需要验证支付宝签名
