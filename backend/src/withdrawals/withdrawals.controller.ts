...
export class WithdrawalsController {
  ...
  async findAll(@Request() req) {
    const withdrawals = await this.withdrawalsService.findAllByUser(
      req.user.userId,
    );
    return {
      success: true,
  ...
  async calculateFee(
    ...
    const withdrawalType =
      type === 'SILVER' ? WithdrawalType.SILVER : WithdrawalType.BALANCE;
    const result = await this.withdrawalsService.calculateWithdrawalFee(
      amountNum,
      withdrawalType,
    );
    return {
