...
### 删除内容
- **用户表字段**: `verifyStatus: number` - 实名认证状态（0=未认证, 1=已认证, 2=审核中）
- **前端筛选**: 用户列表中的"实名状态"筛选下拉框
- **前端显示**: 用户列表中的"实名状态"列
- **编辑功能**: 用户编辑弹窗中的"实名状态"下拉选择框
- **统计数据**: 概览统计中的"已认证用户数"

### 说明
- 用户层面的实名认证已删除，简化用户管理
- **买号实名认证保留**: 买号(BuyerAccount)的实名认证功能不受影响，继续使用

### 相关文件
- `backend/src/users/user.entity.ts` - 删除verifyStatus字段
- `backend/src/users/users-admin.controller.ts` - 删除DTO中的verifyStatus
- `backend/src/users/users-admin.service.ts` - 删除验证相关逻辑和统计
- `frontend/src/app/admin/users/page.tsx` - 删除列、筛选器、编辑表单
- `backend/src/migrations/1768700200000-RemoveUserVerifyStatus.ts` - 数据库迁移
