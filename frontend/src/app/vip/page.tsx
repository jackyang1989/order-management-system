...
function VipContent() {
    ...
    const isBalanceSufficient = () => selectedPackage ? getCurrentBalance() >= selectedPackage.discountPrice : false;
                            ...
                            <div className="space-y-3">
                                {packages.map(pkg => (
                                    <div key={pkg.id} onClick={() => setSelectedPackage(pkg)}
                                        ...
                                        {selectedPackage?.id === pkg.id && <div className="absolute -top-2 right-3 rounded bg-blue-500 px-2 py-0.5 text-xs text-white">已选</div>}
                            ...
                            <div className="space-y-2">
                                {PAYMENT_METHODS.map(method => (
                                    <div key={method.key} onClick={() => setPaymentMethod(method.key)}
                                        ...
                                        {method.key !== 'alipay' && <span className={cn('text-sm font-medium', method.key === 'silver' ? 'text-amber-500' : 'text-slate-700')}>{method.key === 'silver' ? userSilver.toFixed(2) : `¥${userBalance.toFixed(2)}`}</span>}
